...
int sum3(int x, int y, int z) {
  ...
  ***************************************************************/
  ...
  word1 = sum(x , y);
  word2 = sum(word1 , z);
  /* word2 is equals to x+y+z, right shift to divide 2,so add the last bit to word 1 gets the result */
  word2 = word2 >> 1;
  word1 = word2;
  /* word1 += last bit of x+y+z, which was threw by the operator division */
  /* of course the last bit can be got by xor operation */
  word1 += (x^y^z)&0x01;
